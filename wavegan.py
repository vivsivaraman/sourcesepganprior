...
# -*- coding: utf-8 -*-
"""
From : https://github.com/chrisdonahue/wavegan
...
def WaveGANGenerator(
  ...
  output = z
  with tf.variable_scope('z_project', reuse=tf.AUTO_REUSE):
  ...
  # Layer 0
  # [16, 1024] -> [64, 512]
  with tf.variable_scope('upconv_0', reuse=tf.AUTO_REUSE):
  ...
  # Layer 1
  # [64, 512] -> [256, 256]
  with tf.variable_scope('upconv_1', reuse=tf.AUTO_REUSE):
    output = conv1d_transpose(output, dim * dim_mul, kernel_len, 4, upsample=upsample)
    output = batchnorm(output)
  output = tf.nn.relu(output)
  dim_mul //= 2
  ...
  # Layer 2
  # [256, 256] -> [1024, 128]
  with tf.variable_scope('upconv_2', reuse=tf.AUTO_REUSE):
    output = conv1d_transpose(output, dim * dim_mul, kernel_len, 4, upsample=upsample)
    output = batchnorm(output)
  output = tf.nn.relu(output)
  dim_mul //= 2
  ...
  # Layer 3
  # [1024, 128] -> [4096, 64]
  with tf.variable_scope('upconv_3', reuse=tf.AUTO_REUSE):
    output = conv1d_transpose(output, dim * dim_mul, kernel_len, 4, upsample=upsample)
    output = batchnorm(output)
  output = tf.nn.relu(output)
  ...
  if slice_len == 16384:
    # Layer 4
    # [4096, 64] -> [16384, nch]
    with tf.variable_scope('upconv_4', reuse=tf.AUTO_REUSE):
    ...
    # Layer 5
    # [16384, 64] -> [32768, nch]
    with tf.variable_scope('upconv_5', reuse=tf.AUTO_REUSE):
...
def WaveGANDiscriminator(
  ...
  # Layer 1
  # [4096, 64] -> [1024, 128]
  with tf.variable_scope('downconv_1', reuse=tf.AUTO_REUSE):
  ...
  # Layer 2
  # [1024, 128] -> [256, 256]
  with tf.variable_scope('downconv_2', reuse=tf.AUTO_REUSE):
  ...
  # Layer 3
  # [256, 256] -> [64, 512]
  with tf.variable_scope('downconv_3', reuse=tf.AUTO_REUSE):
  ...
  # Layer 4
  # [64, 512] -> [16, 1024]
  with tf.variable_scope('downconv_4', reuse=tf.AUTO_REUSE):
  ...
  if slice_len == 32768:
    # Layer 5
    # [32, 1024] -> [16, 2048]
    with tf.variable_scope('downconv_5', reuse=tf.AUTO_REUSE):
