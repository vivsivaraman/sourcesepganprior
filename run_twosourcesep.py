...
from Utils.util import magnitude_spectrogram, log_magnitude_spectrogram, make_batch, save_audio_two
from evaluate import compute_metrics_two 
   

##################################################################################
def main(args):
    
    # We only consider 2 sources in the mixture
    n_sources = 2
    ...
    #Initializing a list for two z's where the z vectors get updated during every TF iteration
    z_placeholder = []
    ...
    l_exc = compute_exclusion_loss(log_mag_spec_gen_s1, log_mag_spec_gen_s2, level=3)
    ...
    for iter_ in range(args.iterations):
        ...
        # audio_gen_mixture is a list of [gen_s1, gen_s2] which is updated during every iteration
        # audio_true_mixture is an array containing the originally passed input data X (For verification)
        # z is a list of [z_placeholder[0], z_placeholder[1]] which gets updated during every iteration
        ...
        # We store the generated sources s1 and s2 after every 200 iterations.
        # We also save the synthesized mixture as a sum the two generated audio sources
        if ((iter_+1) % 200) == 0: 
            
            save_audio_two(audio_gen_mixture, args, iter_) 
    ...
    compute_metrics_two(X, audio_gen_mixture, args)   
...
if __name__ == '__main__':
    ...
    parser.add_argument('--end', type=int, default = 199, help = 'Ending Mixture Number')
