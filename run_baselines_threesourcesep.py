...
import argparse
from Utils.util import normalize, make_batch, save_audio_three
from evaluate import compute_SDR_SIR_three, compute_SDR_mixture_three, compute_SNR_spec, envelope_two

#All the baseline models require atleast three observations in order to 
...
#The first mixture = s1 + s2 + s3; The second mixture = s1 + s2 + s3 + noise of very small variance  


#Baseline 1 (FastICA)
def compute_fastICA(s1, s2 , s3, args, k):
    
#Inputs : s1 and s2 and s3 are the gt_source signals each of len 16384.
#Outputs : SDRs, SIRs, ENV and Spectral SNR for every example mixture 
    
    #Reshaping s1 and s2 and s3 into (16384, 1), so that we can concatenate along axis =1
    #It is done in order to support the input shape for the sklearn functions
    #source_1 and source_2 and source_3 are of shape [16384, 1]
    source_1 = np.reshape(s1, (args.dim_m, 1))
    source_2 = np.reshape(s2, (args.dim_m, 1))
    source_3 = np.reshape(s3, (args.dim_m, 1))
    
    #Observation 1 (Additive mixing)
    #x1 is of shape [16384, 1]
    x1 = 1.0*source_1 + 1.0*source_2 + 1.0*source_3
    
    #Observation 2 (Additive mixing + noise of small variance)
    #x2 is of shape [16384, 1]
    x2 = 1.0*source_1 + 1.0*source_2 + 1.0*source_3 + args.noise_var*np.random.uniform(args.dim_m, 1)
    
    #Observation 3 (Additive mixing + another noise of small variance)
    #x3 is of shape [16384, 1]
    x3 = 1.0*source_1 + 1.0*source_2 + 1.0*source_3 + 0.01*args.noise_var*np.random.uniform(args.dim_m, 1)
        
    
    #Concatenating along axis 1 to obtain the observation matrix
    #X is of shape [16384, 3]
    X = np.concatenate((x1, x2, x3), axis=1)
    
    #Using sklearn FastICA library (3 components as we have only two sources)
    transformer = FastICA(n_components = 3, random_state=0)
    
    #Obtain the predicted sources
    #X_transformed is of shape [16384, 3]
    ...
    X_transformed[:,2] = normalize(X_transformed[:, 2])
    
    #Transposing X_transformed so that it is in a format that can be used by SDR evaluation funtion
    #X_transformed is of shape [3, 16384]    
    X_transformed = X_transformed.T
    
    #These steps make X_transformed a matrix of shape [3, 1, 16384, 1]
    ...
    save_audio_three(X_transformed, args, k)
    
    #Calling the compute_SDR_SIR_three function to compute SDR and SIR per example
    #Computing Spectral SNR as well as Envelope distance
    #s1 and s2 and s3 are of shape [1, 16384, 1]
    #X_transformed is of shape [3, 1,16384, 1]
    
    sdr_s1, sdr_s2, sdr_s3, sir_s1, sir_s2, sir_s3 = compute_SDR_SIR_three(s1, s2, s3, X_transformed, args)
    sdr_m = compute_SDR_mixture_three((s1+s2+s3), X_transformed, args)    
    ...
    snr_spec_s3 = compute_SNR_spec(s3, X_transformed[2,:,:,:], args, args.sname3)
    snr_spec_mix = compute_SNR_spec(s1+s2, X_transformed[0,:,:,:]+X_transformed[1,:,:,:]+X_transformed[2,:,:,:], args, 'mix')
    
    env_s1 = envelope_two(s1, X_transformed[0,:,:,:], args, args.sname1)
    env_s2 = envelope_two(s2, X_transformed[1,:,:,:], args, args.sname2)
    env_s3 = envelope_two(s3, X_transformed[2,:,:,:], args, args.sname3)
    env_mix = envelope_two(s1+s2+s3, X_transformed[0,:,:,:]+X_transformed[1,:,:,:]+X_transformed[2,:,:,:], args, 'mix')
    
    
    return sdr_s1, sdr_s2, sdr_s3, sir_s1, sir_s2, sir_s3, sdr_m, snr_spec_s1, snr_spec_s2, snr_spec_s3, snr_spec_mix, env_s1, env_s2, env_s3, env_mix



#Baseline 2 (PCA)
def compute_PCA(s1, s2 , s3, args, k):
...
#Outputs : SDRs, SIRs, ENV and Spectral SNR for every example mixture 
    ...
    #Using sklearn PCA library (3 components as we have only three sources)
    ...
    snr_spec_s2 = compute_SNR_spec(s2, X_transformed[1,:,:,:], args, args.sname2)
    snr_spec_s3 = compute_SNR_spec(s3, X_transformed[2,:,:,:], args, args.sname3)
    snr_spec_mix = compute_SNR_spec(s1+s2+s3, X_transformed[0,:,:,:]+X_transformed[1,:,:,:]+X_transformed[2,:,:,:], args, 'mix')
...
def compute_kPCA(s1, s2 , s3, args, k):
#Inputs : s1 and s2 and s3 are the gt_source signals each of len 16384.
#Outputs : SDRs, SIRs, ENV and Spectral SNR for every example mixture 
...
#Baseline 4 (NMF)
def compute_NMF(s1, s2 , s3, args, k):
#Inputs : s1 and s2 and s3 are the gt_source signals each of len 16384.
#Outputs : SDRs, SIRs, ENV and Spectral SNR for every example mixture 
    ...
    source_3_hat = np.expand_dims(np.matmul(W, H[:,2:3]), axis=0)-3.0
        
    #Normalizing estimated source 1 and 2 and 3
    source_1_hat = normalize(source_1_hat)
    source_2_hat = normalize(source_2_hat)
    source_3_hat = normalize(source_3_hat)
    
    #X_transformed has a shape [3, 16384, 1]    
    X_transformed = np.concatenate((source_1_hat, source_2_hat, source_3_hat), axis=0)
    
    #X_transformed now has a shape of [3,1,16384,1]
    X_transformed = np.expand_dims(X_transformed, axis=1)
...
def run_baseline_expt(args):
    ...
    SDR_s1, SDR_s2, SDR_s3, SDR_mix = [], [], [], []
    
    SIR_s1, SIR_s2, SIR_s3 = [], [], []
    
    ENV_s1, ENV_s2, ENV_s3, ENV_mix = [], [], [], []
    
    SNR_SPEC_s1, SNR_SPEC_s2, SNR_SPEC_s3, SNR_SPEC_mix = [], [], [], []
    
    for i in range(gt_s1.shape[0]):
        
        if args.expt_name == 'FastICA':
            sdr_s1, sdr_s2, sdr_s3, sir_s1, sir_s2, sir_s3, sdr_m, snr_spec_s1, snr_spec_s2, snr_spec_s3, snr_spec_mix, env_s1, env_s2, env_s3, env_mix  = compute_fastICA(gt_s1[i:i+1], gt_s2[i:i+1], gt_s3[i:i+1], args, i)
        elif args.expt_name == 'PCA':
            sdr_s1, sdr_s2, sdr_s3, sir_s1, sir_s2, sir_s3, sdr_m, snr_spec_s1, snr_spec_s2, snr_spec_s3, snr_spec_mix, env_s1, env_s2, env_s3, env_mix  = compute_PCA(gt_s1[i:i+1], gt_s2[i:i+1], gt_s3[i:i+1], args, i)
        elif args.expt_name == 'KernelPCA':
            sdr_s1, sdr_s2, sdr_s3, sir_s1, sir_s2, sir_s3, sdr_m, snr_spec_s1, snr_spec_s2, snr_spec_s3, snr_spec_mix, env_s1, env_s2, env_s3, env_mix  = compute_kPCA(gt_s1[i:i+1], gt_s2[i:i+1], gt_s3[i:i+1], args, i)
        elif args.expt_name == 'NMF':
            sdr_s1, sdr_s2, sdr_s3, sir_s1, sir_s2, sir_s3, sdr_m, snr_spec_s1, snr_spec_s2, snr_spec_s3, snr_spec_mix, env_s1, env_s2, env_s3, env_mix  = compute_NMF(gt_s1[i:i+1], gt_s2[i:i+1], gt_s3[i:i+1], args, i)
        
        SDR_s1.append(sdr_s1)
        SDR_s2.append(sdr_s2)
        SDR_s3.append(sdr_s3)
        
        SDR_mix.append(sdr_m)
        
        SIR_s1.append(sir_s1)
        SIR_s2.append(sir_s2)
        SIR_s3.append(sir_s3)
               
        ENV_s1.append(env_s1) 
        ENV_s2.append(env_s2)
        ENV_s3.append(env_s3)
        ENV_mix.append(env_mix)
        
        SNR_SPEC_s1.append(snr_spec_s1)
        SNR_SPEC_s2.append(snr_spec_s2)
        SNR_SPEC_s3.append(snr_spec_s3)
    ...
    print(('Median SDR {} = {}').format(args.sname3, np.nanmedian(SDR_s3)))
    ...
    print(('Median SIR {} = {}').format(args.sname3, np.nanmedian(SIR_s3)))
    print('########')
    ...
    print(('Median Spectral SNR {} = {}').format(args.sname3, np.nanmedian(SNR_SPEC_s3)))
    ...
    print(('Median Envelope Distance: {} = {}').format(args.sname3, np.nanmedian(ENV_s3)))
...
parser.add_argument('--expt_name', type=str, default = 'PCA', help = 'Experiment Name')
